...
@section('title')
    Registro Compra Insumos
@endsection

@section('content')
    @include('tenant.consumables.purchases.modals.mdl_consumables')
    @include('tenant.consumables.purchases.modals.mdl_edit_item')
    ...
    <div class="card">
        ...
        <div class="card-body">
            @include('tenant.consumables.purchases.forms.form_create')
        </div>
        <div class="card-footer d-flex justify-content-between align-items-center">
            ...
            <div style="display:flex;">
                ...
                <button class="btn btn-primary" type="submit" form="formStorePurchase">
...
@section('js')
    <script>
        ...
        function events() {
            ...
            document.querySelector('#formStorePurchase').addEventListener('submit', (e) => {
                e.preventDefault();
                const validacion = validacionRegistrarCotizacionCompra();
                if (validacion) {
                    storePurchase(e.target);
                }
            })
        ...
        function storePurchase(formStore) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const url = @json(route('tenant.insumos.compras.store'));
                    ...
                    Swal.fire({
                        title: 'Cargando...',
                        html: 'Registrando compra de insumos...',
                    ...
                    try {
                        ...
                        if (res.success) {
                            const purchase_document_index = @json(route('tenant.insumos.compras.index'));
                            toastr.success(res.message, 'OPERACIÓN COMPLETADA');
                            window.location.href = purchase_document_index;
                        } else {
                            toastr.error(res.message, 'Error en el servidor registrar compra de insumos');
                            Swal.close();
                        }


                    } catch (error) {
                        toastr.error(error, 'Error en la petición registrar compra de insumos');
                        Swal.close();
                    }
