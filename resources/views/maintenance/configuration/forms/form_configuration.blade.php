                ...
                <div class="col-lg-6 col-md-6 col-sm-6">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <x-toggle-switch-1 id="configuration_{{ $item->id }}"
                            name="configuration_{{ $item->id }}" :checked="$item->status == 1" />

                        {{-- Input group password --}}
                        <div class="input-group" style="max-width: 240px;">
                            <input value="{{$item->property}}" type="password" class="form-control input-fill"
                                id="configuration_password_{{ $item->id }}"
                                name="configuration_password_{{ $item->id }}" placeholder="Contraseña">

                            <span class="input-group-text bg-light toggle-password" style="cursor: pointer;"
                                data-target="configuration_password_{{ $item->id }}">
                                <i class="fas fa-lock text-warning"></i>
                            </span>
                        </div>
                    </div>

                    <p class="configuration_password_{{ $item->id }}_error msgError"></p>
                </div>
            @endif
