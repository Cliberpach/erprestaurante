    ...
    <div class="card overflow-hidden">
        <div class="card-header">
            ...
            <div class="row mb-2 justify-between">
                <div class="col-6">
                    <h6 class="card-title mb-0">LISTA DE NOTAS DE CRÉDITO</h6>
                </div>
...
@section('js')
    <script>
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            const urlDownloadXml =
                                "{{ route('tenant.ventas.notas_credito.downloadXml', ':id') }}".replace(
                                    ':id', data.id);
                            const urlDownloadCdr =
                                "{{ route('tenant.ventas.notas_credito.downloadCdr', ':id') }}".replace(
                                    ':id', data.id);
                            const urlPdf =
                                "{{ route('tenant.ventas.notas_credito.pdfOne', ':id') }}".replace(
                                    ':id', data.id);
        ...
        async function sendSunat(creditNoteId) {

            const note = getRowById(dtNotes, creditNoteId);

            let message = `Enviar la nota de crédito: ${note.serie}-${note.correlative} a Sunat?`;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        title: `Documento de venta: ${note.serie}-${note.correlative}`,
                    ...
                    try {
                        ...
                        const url = @json(route('tenant.ventas.notas_credito.send_sunat'));
