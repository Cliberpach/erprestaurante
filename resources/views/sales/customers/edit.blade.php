...
<script>
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    let url = `{{ route('tenant.ventas.clientes.update', ['id' => ':id']) }}`;
    ...
    async function consultarDocumento(tipo_documento, nro_documento) {
        ...
        try {
            ...
            const url =
                `{{ route('tenant.ventas.clientes.consult_document') }}?type_identity_document=${encodeURIComponent(tipo_documento)}&nro_document=${encodeURIComponent(nro_documento)}`;
...
*/
    ...
    function fijarUbigeo() {
        ...
        if (cliente.department_id) {
            const departmentId = String(parseInt(cliente.department_id));
            window.departmentSelect.setValue(departmentId);
        }
        if (cliente.province_id) {
            const provinceId = String(parseInt(cliente.province_id));
            window.provinceSelect.setValue(provinceId);
        }
        if (cliente.district_id) {
            const districtId = String(parseInt(cliente.district_id));
            window.districtSelect.setValue(districtId);
        }
    }
</script>
