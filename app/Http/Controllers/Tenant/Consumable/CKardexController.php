...
class CKardexController extends Controller
{
    ...
    {
        return view('tenant.consumables.kardex.index');
    }
    ...
    public static function queryKardex(Request $request)
    {
        ...
        $consumable_id  =   $request->get('consumable_id');
        $start_date     =   $request->get('start_date');
        $end_date       =   $request->get('end_date');

        $kardex =   DB::select(
            'CALL sp_kardex_consumable(?, ?, ?,?)',
            [$warehouse_id, $consumable_id, $start_date, $end_date]
        );
  ...
  "sale_document_id"    => 43
    ...
    */
    ...
    public function pdf(Request $request)
    {
        ...
        $pdf = Pdf::loadview('tenant.consumables.kardex.pdf.pdf', [
            'company'               =>  $company,
            'data'                  =>  $data,
            'filters'               =>  $request
        ...
        return $pdf->stream('kardex_insumos' . Carbon::now()->format('Y_m_d_H_i_s') . '.pdf');
    }
}
