...
class KardexDto
{
    ...
    public function getDtoFromNoteRelease(NoteRelease $note)
    {
        $dto            =   [];
        $lst_detail     =   NoteReleaseDetail::where('note_release_id', $note->id)->get();
        ...
        foreach ($lst_detail as $item) {
            ...
            $_item                          =   [];
            $_item['note_release_id']        =   $note->id;
            $_item['type']                  =   'SALIDA';
            $_item['document_serie']        =   'NS-' . str_pad($note->id, 8, '0', STR_PAD_LEFT);
            $_item['date']                  =   $note->created_at;
            $_item['warehouse_id']          =   $item->warehouse_id;
            $_item['warehouse_name']        =   $item->warehouse_name;
            $_item['product_id']            =   $item->product_id;
    ...
    public function getDtoFromSale(Sale $sale)
    {
        ...
        foreach ($lst_detail as $item) {
            ...
            $_item['brand_id']              =   $item->brand_id;
            $_item['product_name']          =   $item->product_name;
            $_item['product_unit']          =   $product->unit_symbol;
            $_item['category_name']         =   $item->category_name;
            $_item['brand_name']            =   $item->brand_name;
            $_item['quantity']              =   $item->quantity;
            ...
            $_item['created_at']            =   Carbon::now();
            $_item['creator_user_id']       =   $sale->creator_user_id;
            $_item['creator_user_name']     =   $sale->creator_user_name;
    ...
    public function getDtoFromOrder(Order $order)
    {
        $dto            =   [];
        $lst_detail     =   OrderProduct::where('order_id', $order->id)->where('status', '<>', 'ANULADO')->get();
        ...
        foreach ($lst_detail as $item) {
            ...
            $_item['type']                  =   'SALIDA';
            $_item['document_serie']        =   $order->code;
            $_item['date']                  =   $item->created_at;
            $_item['warehouse_id']          =   $item->warehouse_id;
            $_item['warehouse_name']        =   $warehouse->descripcion;
            $_item['product_id']            =   $item->product_id;
            $_item['category_id']           =   $item->category_id;
            ...
            $_item['amount']                =   $item->total;
            $_item['created_at']            =   Carbon::now();
            $_item['creator_user_id']       =   $order->creator_user_id;
            $_item['creator_user_name']     =   $order->creator_user_name;
            $_item['customer_id']                           =   $order->customer_id;
            $_item['customer_name']                         =   $order->customer_name;
            $_item['customer_type_document_abbreviation']   =   $order->customer_type_document_abbreviation;
            $_item['customer_document_number']              =   $order->customer_document_number;
